...
class AgentHandler:
    ...
    def next_action(self, opponent_action):
        if opponent_action is not None:
            ...
            self._time_step = self._exec_step(self._time_step,action_step,own_turn=False,self._buffer)
    
    def _exec_step(self,time_step,action_step,own_turn,buffer):
        self._environment.set_next_is_agent_or_opponent(is_agent = own_turn)
        next_time_step = self._environment.tf.step(action_step.action)
        if own_turn:
